...
@section('title', 'Aprovacao de Farmácias - Medlink')

@section('content')
    <div class="glass rounded-3xl p-8">
        <div class="flex flex-wrap items-start justify-between gap-6">
            <div>
                <h1 class="brand-title text-3xl text-slate-900">Aprovacao de Farmácias</h1>
                <p class="mt-2 text-sm text-slate-600">Gerencie solicitacoes pendentes e historico.</p>
            </div>
        ...
        <div class="mt-8">
            ...
            @if ($pending->isEmpty())
                <p class="mt-3 text-sm text-slate-500">Nenhuma solicitacao pendente.</p>
            @else
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-white/90 p-6">
                            <div class="flex flex-wrap items-start justify-between gap-4">
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-slate-600">{{ $pharmacy->responsible_name }} · {{ $pharmacy->phone }}</p>
            ...
            <div class="mt-4 overflow-hidden rounded-2xl border border-slate-200">
                <table class="min-w-full divide-y divide-slate-800 text-sm">
                    <thead class="bg-white/90 text-slate-500">
                        <tr>
                            <th class="px-4 py-3 text-left">Farmácia</th>
                    ...
                    <tbody class="divide-y divide-slate-800 bg-white/40 text-slate-700">
                        @foreach ($pharmacies as $pharmacy)
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    <span class="rounded-full border border-slate-300 px-3 py-1 text-xs uppercase tracking-[0.2em]">
                                        {{ $pharmacy->status }}
                                    </span>
                                </td>
