...
@section('title', 'Cadastro de Farmácia - Medlink')

@section('content')
    <div class="glass rounded-3xl p-8">
        <div class="flex flex-wrap items-start justify-between gap-6">
            <div>
                <h1 class="brand-title text-3xl text-slate-900">Cadastro da Farmácia</h1>
                <p class="mt-2 text-sm text-slate-600">
                    Preencha os dados da Farmácia. O admin precisa aprovar antes de ativar.
                </p>
            </div>
        ...
        <form class="mt-8 grid gap-5 md:grid-cols-2" method="POST" action="{{ route('pharmacy.store') }}">
            @csrf
            ...
            <div>
                ...
                <input class="mt-2 w-full rounded-2xl border border-slate-300 bg-white/80 px-4 py-3 text-sm text-slate-900 placeholder:text-slate-500 focus:border-lime-400 focus:outline-none" type="text" name="nif" value="{{ old('nif') }}" placeholder="Ex: 5000000000" required />
            </div>
            <div>
                <label class="text-xs uppercase tracking-[0.2em] text-slate-500">Telefone</label>
                <input class="mt-2 w-full rounded-2xl border border-slate-300 bg-white/80 px-4 py-3 text-sm text-slate-900 placeholder:text-slate-500 focus:border-lime-400 focus:outline-none" type="text" name="phone" value="{{ old('phone') }}" placeholder="(+244) 900 000 000" required />
            </div>
            <div>
